...
function filter(phrase, clearBtnId, tableId) {
    ...
    if (phrase.value.length != 0) {
        ...
        if (elt == null) {
            ...
            cell_total.innerText = getSize(sums[2]);

            var cell_totaldown = row.insertCell(3);
            cell_totaldown.className = "coltotal";
            cell_totaldown.innerHTML = getSize(sums[3]) + ' &#x25BC';

            var cell_totalup = row.insertCell(4);
            cell_totalup.className = "coltotal";
            cell_totalup.innerHTML = getSize(sums[4]) + ' &#x25B2';

            var cell_averagetotal = row.insertCell(5);
            cell_averagetotal.className = "colavg";
            cell_averagetotal.innerText = getSize(sums[5]);

            var cell_averagedown = row.insertCell(6);
            cell_averagedown.className = "colavg";
            cell_averagedown.innerHTML = getSize(sums[6]) + ' &#x25BC';

            var cell_averageup = row.insertCell(7);
            cell_averageup.className = "colavg";
            cell_averageup.innerHTML = getSize(sums[7]) + ' &#x25B2';

            var cell_todaytotal = row.insertCell(8);
            cell_todaytotal.className = "coltoday";
            cell_todaytotal.innerText = getSize(sums[8]);

            var cell_todaydown = row.insertCell(9);
            cell_todaydown.className = "coltoday";
            cell_todaydown.innerHTML = getSize(sums[9]) + ' &#x25BC';

            var cell_todayup = row.insertCell(10);
            cell_todayup.className = "coltoday";
            cell_todayup.innerHTML = getSize(sums[10]) + ' &#x25B2';

            var cell_lastupdate = row.insertCell(11);
        }
        // If the Filter Totals row is showing already, just update the values
        else {
            var cells = elt.getElementsByTagName("td");
            cells[2].innerText = getSize(sums[2]);
            cells[3].innerHTML = getSize(sums[3]) + ' &#x25BC';
            cells[4].innerHTML = getSize(sums[4]) + ' &#x25B2';
            cells[5].innerText = getSize(sums[5]);
            cells[6].innerHTML = getSize(sums[6]) + ' &#x25BC';
            cells[7].innerHTML = getSize(sums[7]) + ' &#x25B2';
            cells[8].innerText = getSize(sums[8]);
            cells[9].innerHTML = getSize(sums[9]) + ' &#x25BC';
            cells[10].innerHTML = getSize(sums[10]) + ' &#x25B2';
        }
    } else {
